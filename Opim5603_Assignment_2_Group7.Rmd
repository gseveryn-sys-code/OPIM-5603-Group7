...
Define A = liking science fiction (SC) movies
Define B = liking role playing (RP) games
...
p_SF <- 0.1 #given
p_RP <- 0.2 #given
p_NotSF_and_NotRP <- 0.75 #given

p_NotSF <- 1-p_SF #not is 1-x for mutually exclusive
p_NotRP <- 1-p_RP #not is 1-x for mutually exclusive

p_NotSF_and_RP <- p_NotSF - p_NotSF_and_NotRP
p_SF_and_NotRP <- p_NotRP - p_NotSF_and_NotRP

p_SF_and_RP <- p_SF - p_SF_and_NotRP
p_SF_and_RP
...
ggplot(df, aes(x = B, y = A)) +
  ...
  labs(title = "Joint Probability", x = "Event B", y = "Event A") +
  theme_minimal()

#Reprint Answer to Question
p_SF_and_RP
```
...
pA <- 0.1
if(pAgivenB != pA){
  print("Liking science fiction movies and role playing games are not independent because the conditional probability of liking science fiction given liking role playing is not equal to the marginal probability of liking science fiction")
}
```
...
unstableGeoMean <- function(x, na.rm = FALSE) {
  if (na.rm) {
    ...
    stop("x contains NA; set na.rm = TRUE to drop them.")# removes the NAs
  }
...
```{r}
stableGeoMean <- function(x, na.rm = FALSE) {
  if (na.rm) {
    x <- x[!is.na(x)]
  } else if (anyNA(x)) {
    stop("x contains NA; set na.rm = TRUE to drop them.") # removes the NAs
...
```{r}
x.10 <- 1:10
unstable <- unstableGeoMean(x.10)
stable <- stableGeoMean(x.10)
...
```{r}
x.1000 <- 1:1000
largeUnstable <- unstableGeoMean(x.1000)
largeStable <- stableGeoMean(x.1000)

print(largeUnstable)
print(largeStable)

# Computers store numbers with limited range so for the unstable geometric mean it becomes inf before we are able to take the root. Summing logs stays within range of computer memory.
```
